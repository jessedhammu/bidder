...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!check_csrf($_POST['csrf'] ?? '')) $err = 'Invalid CSRF token';
        ...
        $p = $_POST['password'] ?? '';
        if (login_user($u, $p)) {
            if ($_SESSION['user']['role'] !== 'admin') { logout(); $err = 'Not an admin account'; }
            else { audit_log('LOGIN_SUCCESS','auth',null,null,['username'=>$u]); header('Location: admin/dashboard.php'); exit; }
